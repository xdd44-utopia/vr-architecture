...
public class ModelTransform : MonoBehaviour
{
	...
	private const float enableScale = 0.04f;
	private const float disableScale = 0.0025f;
	private const float enableHandleRadius = 0.4f;
	...
	void Update()
	{
		...
		Vector3 target = headTransform.position + dist * new Vector3(Mathf.Sin(headTransform.rotation.eulerAngles.y * Mathf.PI / 180), -1f, Mathf.Cos(headTransform.rotation.eulerAngles.y * Mathf.PI / 180));
